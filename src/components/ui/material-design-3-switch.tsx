...
const switchVariants = cva(
  "peer inline-flex shrink-0 cursor-pointer items-center rounded-full border-2 transition-colors duration-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background disabled:cursor-not-allowed disabled:opacity-50",
  {
    variants: {
      variant: {
        primary: "peer-checked:bg-[#163A5F] peer-checked:border-[#163A5F]",
        destructive: "peer-checked:bg-[#C94F4F] peer-checked:border-[#C94F4F]",
      },
      size: {
...
const Switch = React.forwardRef<HTMLInputElement, SwitchProps>(
  ({ 
    ...
    return (
      <label 
        className={cn(
          "group relative inline-flex items-center justify-center font-sans", 
          ...
          >
            ...
            <div
              className={cn(
                ...
                "bg-white", // Solid White Handle
                
                isChecked 
                  ? (variant === "destructive" ? "text-[#C94F4F]" : "text-[#163A5F]")
                  : "text-[#6B7280]", // Unchecked state icon color matching brand secondary text
            ...
            >
              ...
              {shouldRenderIcons && (
                <div className="relative w-full h-full flex items-center justify-center overflow-hidden">
                    ...
                    <div className={cn(
                        "absolute inset-0 flex items-center justify-center transition-all duration-300 text-[#6B7280]",
                        !isChecked ? "opacity-100 scale-100 rotate-0" : "opacity-0 scale-50 rotate-45"
                    )}>
                        {uncheckedIcon ? (
                             uncheckedIcon
            ...
            <div 
                className={cn(
                    ...
                    isChecked 
                      ? (variant === 'destructive' ? "bg-[#C94F4F]" : "bg-[#163A5F]") 
                      : "bg-[#6B7280]",
